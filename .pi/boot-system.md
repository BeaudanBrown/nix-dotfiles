...
- If `.pi/boot-next-loop.md` is present, treat it as an explicitly seeded advisory only. Validate and record the reboot result first; run the suggested `/aloop 1 ...` only if runtime evidence is captured, the worktree is clean, and no blocker remains.
