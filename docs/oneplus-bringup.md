...
- Best known mic/app exposure path: keep ACP/UCM speaker-only, program the proven bottom-mic ALSA controls with `amixer`, then load `module-alsa-source` for `hw:O6T,1` as `oneplus_bottom_mic`. This was confirmed live on generation 70 and is prepared for generation 71 via `oneplus-mic-source.service`.
