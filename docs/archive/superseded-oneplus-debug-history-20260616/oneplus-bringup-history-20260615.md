...
- UCM for `O6T`, `hw:0`, `oneplus-OnePlus6T`, and `sdm845` loads and intentionally contains only the speaker device. Do **not** reintroduce the previous UCM `Mic1` capture/duplex device: it correlated with exact-zero captures. The working direction is speaker-only UCM plus a separate manual raw ALSA route for the bottom mic.
...
- Best known mic/app exposure path: keep ACP/UCM speaker-only, program the proven bottom-mic ALSA controls with `amixer`, then load `module-alsa-source` for `hw:O6T,1` as `oneplus_bottom_mic`. This was confirmed live on generation 70, but automatic boot-time loading in generation 71 reproduced exact-zero capture. Keep `oneplus-mic-source.service` manual-only until a clean speaker-only boot has first been verified non-zero.
...
- Public downstream/vendor clues found:
  ...
  - Android/OxygenOS source/artifact inspection:
    ...
    - Full power-off/cold-boot retrace on generation 63 reproduced non-zero bottom mic capture and was recorded through the traceable trial wrapper in commit `19c4918` (`docs/oneplus-audio-trials/20260614T124231Z-coldboot-pmos-runtime.md`, artifact `/tmp/oneplus-mic-debug-coldboot-pmos-runtime-20260614T124231Z`). Before capture, PipeWire/WirePlumber was healthy for playback: UCM loaded for `O6T`, default sink was `alsa_output.platform-sound.HiFi__Speaker__sink`, and there was no Pro Audio fallback. Capture results: S16 max `0.005920`, RMS `0.000826`; S24 max `0.004486`, RMS `0.000360`; both reached `RUNNING` with zero fresh q6/AFE dmesg lines. This strongly supports reset depth / opaque ADSP-SLIM-codec runtime state as the working-state variable.
    - An immediate same-boot repeat after the cold-boot success also stayed non-zero and was recorded in commit `ffc6fcb` (`docs/oneplus-audio-trials/20260614T124329Z-sameboot-repeat-pmos-runtime.md`, artifact `/tmp/oneplus-mic-debug-sameboot-repeat-pmos-runtime-20260614T124329Z`). Results: S16 max `0.014252`, RMS `0.000421`; S24 max `0.006073`, RMS `0.000498`. The good state therefore persists across at least one route reset and repeated PMOS runtime capture within the same boot.
    - Generation 64 reintroduced ACP-compatible UCM `Mic1` using the proven `CapturePCM "hw:O6T,1"` route, but PipeWire/WirePlumber still exposed only the UCM Speaker sink and monitor source. `alsaucm -c O6T dump text` showed `Device.Mic1`, while Pulse/ACP profile data showed only `HiFi (Speaker)` with `sources: 0`; restarting PipeWire/WirePlumber did not expose a mic source. The traceable direct ALSA trial in commit `e02921d` (`docs/oneplus-audio-trials/20260614T125257Z-gen64-coldboot-ucm-mic.md`, artifact `/tmp/oneplus-mic-debug-gen64-coldboot-ucm-mic-20260614T125257Z`) also recorded exact-zero for S16 and S24 in that boot. Next experiment is to bypass ACP source synthesis by adding a static raw PipeWire source for `hw:O6T,1` while keeping ACP/UCM Speaker playback.
    - Generation 65 attempted that static raw PipeWire source (`context.objects` adapter, `api.alsa.pcm.source`, `api.alsa.path = "hw:O6T,1"`) and failed hard: PipeWire tried to open the ALSA capture PCM during context creation, got `capture open failed: Invalid argument`, then aborted with `can't create object from factory adapter`, leaving PipeWire/Pulse/WirePlumber down. The same generation also showed that splitting `alsaucm -c O6T set _verb HiFi set _enadev Speaker` into separate invocations is invalid for this setup: `_enadev=Speaker` failed with `No such file or directory`, likely because UCM state is not retained across separate `alsaucm` processes. Commit `5599cc3` reverts the static-source experiment and restores the one-shot UCM speaker command shape.
    - Generation 66 restored PipeWire speaker playback. It still exposed no ACP mic source even though `alsaucm` showed `Mic1`. A traceable direct ALSA trial (`docs/oneplus-audio-trials/20260614T131419Z-gen66-restore-ucm-mic.md`, artifact `/tmp/oneplus-mic-debug-gen66-restore-ucm-mic-20260614T131419Z`) was exact-zero for S16 and S24. A live-only Pulse module test succeeded in exposing an app-visible source after PipeWire was already running: `alsaucm -c O6T set _verb HiFi set _enadev Speaker set _enadev Mic1` followed by `pactl load-module module-alsa-source device=hw:O6T,1 source_name=oneplus_bottom_mic format=s16le rate=48000 channels=1`. `wpctl` then showed source `oneplus_bottom_mic`, but capture from that source was exact-zero in this boot, matching direct ALSA. This proves the app-visible source can be created safely if loaded after startup; it does not solve the lower-level zero/non-zero ADSP state.
    - Generation 67 booted with the delayed Pulse source service and did not fall back to Pro Audio. However enabling `Mic1` before WirePlumber profile creation made ACP choose `HiFi (Mic1)` (`priority: 8101`) instead of `HiFi (Speaker)` (`priority: 8100`), so the default sink became the fake `alsa_output.platform-sound.HiFi__Mic1__sink`. It did expose both the ACP mic source and the explicit `oneplus_bottom_mic` source. A traceable direct ALSA trial (`docs/oneplus-audio-trials/20260614T132446Z-gen67-auto-source-boot.md`, artifact `/tmp/oneplus-mic-debug-gen67-auto-source-boot-20260614T132446Z`) remained exact-zero. Generation 68 changes the boot route back to Speaker-only before WirePlumber starts, then enables `Mic1` only inside the delayed Pulse source service.
    - Generation 68 still selected `HiFi (Mic1)` at boot because the delayed service could run before WirePlumber profile selection settled. A live `pactl set-card-profile alsa_card.platform-sound 'HiFi (Speaker)'` restored `alsa_output.platform-sound.HiFi__Speaker__sink` while preserving the explicit `oneplus_bottom_mic` source. The traceable trial after live speaker-profile recovery (`docs/oneplus-audio-trials/20260614T133412Z-gen68-speaker-profile-recovered.md`, artifact `/tmp/oneplus-mic-debug-gen68-speaker-profile-recovered-20260614T133412Z`) remained exact-zero. Generation 69 persists the same profile correction in `oneplus-mic-source.service` after loading the explicit Pulse source.
    - Generation 69 achieved the intended PipeWire/app-visible stack at boot: no Pro Audio fallback, `alsaucm -c O6T dump text` parses `Speaker` and `Mic1`, default sink is `alsa_output.platform-sound.HiFi__Speaker__sink`, and default source is explicit `oneplus_bottom_mic`. A traceable direct ALSA trial (`docs/oneplus-audio-trials/20260614T134323Z-gen69-speaker-and-app-mic.md`, artifact `/tmp/oneplus-mic-debug-gen69-speaker-and-app-mic-20260614T134323Z`) still recorded exact-zero for S16 and S24. At this point the app stack shape is stable enough; remaining work should focus on the lower-level exact-zero vs non-zero capture state.
    - Targeted comparison between the last non-zero artifact (`/tmp/oneplus-mic-debug-sameboot-repeat-pmos-runtime-20260614T124329Z`) and gen69 zero artifact found identical active `/proc/asound` and DAPM route state after filtering runtime counters. The only notable q6/dmesg difference during S16 capture was the non-zero run logging `wcd934x-codec ... Port Closed TX port 7, value 6`, while gen69 logged only another `SLIM SAT: Rcvd master capability`. Since the last non-zero run was with speaker-only UCM and the first subsequent zero run followed adding persistent `Mic1`, generation 70 temporarily removes `Mic1` from UCM and disables the automatic `oneplus_bottom_mic` source service. The direct debug route still programs the proven bottom-mic controls itself; this retraces whether persistent UCM/source setup affects lower-level capture.
    - Generation 70 restored non-zero direct bottom-mic capture with speaker-only UCM: `docs/oneplus-audio-trials/20260614T135658Z-gen70-speaker-only-ucm-retrace.md` (`/tmp/oneplus-mic-debug-gen70-speaker-only-ucm-retrace-20260614T135658Z`) recorded `S16_LE max=0.003815 rms=0.000192` and `S24_LE max=0.004486 rms=0.000775`. Same-boot repeat `docs/oneplus-audio-trials/20260614T135748Z-gen70-speaker-only-sameboot-repeat.md` (`/tmp/oneplus-mic-debug-gen70-speaker-only-sameboot-repeat-20260614T135748Z`) also stayed non-zero. This strongly implicates the persistent `Mic1` UCM/app-source path, not boot randomness alone. A live app-source test on gen70 then manually set the same raw ALSA controls and loaded `module-alsa-source` without reintroducing UCM `Mic1`; recording from `oneplus_bottom_mic` was non-zero (`/tmp/oneplus-live-gen70-app-source-20260614T235843Z`, `S16_LE max=0.002197 rms=0.000190`). A user-confirmed direct listen test recorded 6 seconds from the bottom mic and played it back through the Speaker sink successfully: `/tmp/oneplus-mic-listen-test-20260615T000248Z/test.wav`, `S16_LE max=0.065277 rms=0.004075`, `277377/288000` non-zero samples.
    - Generation 71 kept speaker-only UCM and restored app exposure through a boot-time manual-route `oneplus-mic-source.service`. The PipeWire shape was ideal (`alsa_output.platform-sound.HiFi__Speaker__sink` default sink and explicit `oneplus_bottom_mic` default source), but app-source capture, direct `hw:O6T,1` capture, and traceable PMOS runtime trial were exact-zero (`docs/oneplus-audio-trials/20260615T004323Z-gen71-manual-source-boot-zero.md`, artifact `/tmp/oneplus-mic-debug-gen71-manual-source-boot-zero-20260615T004323Z`). Unloading the Pulse module and rerouting live did not recover non-zero capture in the same boot. Generation 72 disables automatic `oneplus-mic-source.service` startup again; next boot should retrace direct PMOS runtime capture before any app source is loaded.
...
- Current conclusion: routing to the ALSA capture frontend works and powers the codec path. The stream is non-zero after at least some full power-off/cold boots, but can be exact-zero after warm/retrace boots with no visible ALSA/DAPM difference. This looks lower than UCM/PipeWire now: likely ADSP/AFE/SLIM/codec runtime initialization or reset-depth behavior.
